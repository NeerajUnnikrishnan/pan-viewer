...
export default class LivePlan extends React.Component<LivePlanProps, LivePlanState> {
   ...
   refreshLoop() {
    ...
    updateD3Stage(stage: StageNodeInfo, graph: any, allStages: Map<string, StageNodeInfo>) {
        ...
        stage.nodes.forEach(node => {
            ...
            node.sources.forEach(source => {
                graph.setEdge("node-" + source, nodeId, {arrowheadClass: "plan-arrowhead"});
            });
    ...
    updateD3Graph() {
        ...
        const width = parseInt(window.getComputedStyle(document.getElementById("live-plan"), null).getPropertyValue("width").replace(/px/, "")) + 1000;
        const height = parseInt(window.getComputedStyle(document.getElementById("live-plan"), null).getPropertyValue("height").replace(/px/, "")) + 1500;

        const graphHeight = graph.graph().height + 1000;
        const graphWidth = graph.graph().width + 1000;
        if (this.state.ended) {
