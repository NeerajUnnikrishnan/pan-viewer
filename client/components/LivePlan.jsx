    ...
    render() {
        return (
            <div style={{color: "#000"}} data-toggle="tooltip" data-placement="bottom" data-container="body" data-html="true"
                 title={ this.props.name + "->" + this.props.identifier}>
...
export default class LivePlan extends React.Component<LivePlanProps, LivePlanState> {
    timeoutId: TimeoutID;
   ...
   refreshLoop() {
           ...
           this.state.query = rawdata;
           this.state.initialized = true;
           this.state.ended = this.state.query.finalQueryInfo;
           this.resetTimer();
           fetch("../student.json")
    ...
    static handleStageClick(stageCssId: string) {
        window.open("./client/stage.html?" + stageCssId);
    }
    ...
    updateD3Stage(stage: StageNodeInfo, graph: any, allStages: Map<string, StageNodeInfo>) {
        ...
        graph.setNode(stageRootNodeId, {label: html, style: 'fill: #fff', labelType: "html"});
        graph.setParent(stageRootNodeId, clusterId);
        graph.setEdge("node-" + stage.root, stageRootNodeId, {style: "stroke:#eaece5;stroke-width:3px;visibility: hidden;"});
        ...
        stage.nodes.forEach(node => {
            ...
            node.sources.forEach(source => {
                graph.setEdge("node-" + source, nodeId, {class: "plan-edge", arrowheadClass: "plan-arrowhead", style: "stroke:#eaece5;stroke-width:3px;",labelStyle: "color: red; font-weight: bold; font-size: 24px;"});
            });
                ...
                node.remoteSources.forEach(sourceId => {
                    ...
                    if (source) {
                        ...
                        graph.setEdge("stage-" + sourceId + "-root", nodeId, {
                                ...
                                labelStyle: "color: #111; font-weight: bold; font-size: 24px;",
                                labelType: "html",
                        });
                    }
                });
